...
import {createIndependentRecoveryReceipt} from './recovery-receipt.js';

const TOOL_VERSION='1.1.0';
const app=document.querySelector('#app');
const stages=['读取恢复材料','获取加密资料','验证资料完整性','解密 Recovery Map','恢复附件','恢复完成'];
let recoveryInFlight=false;
const toolManifest=JSON.parse(document.querySelector('#tool-manifest').textContent);
...
function renderRecovered(snapshot,receipt){const result=document.querySelector('#result'),outputHash=snapshot.integrity.snapshot_payload_sha256;document.querySelector('.recovery-entry').hidden=true;result.innerHTML=`<section class="card recovery-complete"><p class="eyebrow">RECOVERY COMPLETE</p><h2>你的 Recovery Map 已成功恢复</h2><p>资料已完成验证、解密和附件恢复。</p><p class="verification-result">恢复结果精确匹配：PASS<br><small>Snapshot SHA-256：${outputHash}</small></p><button id="download-recovery-receipt">保存非秘密恢复凭证</button> <button id="enter-recovery-map">进入 Recovery Map</button></section><div id="recovered-map" hidden></div>`;document.querySelector('#download-recovery-receipt').onclick=()=>{const bytes=new TextEncoder().encode(JSON.stringify(receipt,null,2)),url=URL.createObjectURL(new Blob([bytes],{type:'application/json'})),link=document.createElement('a');link.href=url;link.download=`LEGAVIK-Recovery-Receipt-${receipt.pairing_identifier}.json`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};document.querySelector('#enter-recovery-map').onclick=()=>{const map=result.querySelector('#recovered-map');map.hidden=false;renderRecoveryMap(map,snapshot,{allowAttachmentDownloads:true});map.querySelectorAll('[data-recovery-guide-action]').forEach((link)=>link.remove());map.querySelectorAll('.recovery-guide-actions').forEach((group)=>{if(!group.children.length)group.remove();});result.querySelector('.recovery-complete').hidden=true;progressCard.hidden=true;map.scrollIntoView({behavior:'auto',block:'start'});};}
...
recoverButton.onclick=async()=>{
  ...
  setRecovering(true);
  try{
    stage(0);const evidenceBytes=new Uint8Array(await evidenceFile.arrayBuffer()),evidence=validateMainnetEvidence(JSON.parse(new TextDecoder().decode(evidenceBytes))),kitBytes=new Uint8Array(await kitFile.arrayBuffer()),kit=recoveryKitBuilder.parseKit(kitBytes);assertKitEvidencePair({kit,evidence});
    ...
    stage(3);const recovered=await recoverVaultArtifacts({kitBytes,archiveBytes,password});if(evidence.source_sha256&&recovered.snapshot.integrity.snapshot_payload_sha256!==evidence.source_sha256)throw new Error('恢复结果与创建时记录的 Snapshot SHA-256 不一致。');const receipt=await createIndependentRecoveryReceipt({kitBytes,evidenceBytes,archiveBytes,evidence,snapshot:recovered.snapshot,toolManifest});stage(4);renderRecovered(recovered.snapshot,receipt);stage(5);message.className='success';message.textContent='恢复成功。Recovery Map、恢复结果哈希与全部附件完整性验证通过。';
